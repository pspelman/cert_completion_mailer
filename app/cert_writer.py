...
def create_certificate(
    ...
    if certificate_type == "indigenized_MI".upper():
        new_docx_file = f"{docx_output_dir}/HaRT3S-Indigenized-MI-certificate-{attendee_name.replace(' ', '-')}.docx"
    else:
    ...
    new_docx = zipfile.ZipFile(
        new_docx_file, "w"
    )  # X for exclusive create mode | a for append | w for write
    with open(docx_template.extract("word/document.xml", "./")) as tempXmlFile:
        ...
        for key in replace_text_fields.keys():
            ...
            print(
                f"FINDING in XML_TEMPLATE_STRING: ", xml_template_string.find(str(key))
            )
            xml_template_string = xml_template_string.replace(
                str(key), str(replace_text_fields.get(key))
            )
...
class AttendeeTracker:
    ...
    def load_attendees(self) -> list:
            ...
            for n, row in enumerate(line_reader, 0):
                if n and row:  # skip blank rows
                    try:
                        ...
                        if (
                            attendee_email in self.emails
                            or attendee_email in new_emails
                        ):  # Do not allow duplicate email addresses, each attendee should have a unique email
                            raise ValueError(
                        ...
                        print(
                            f"Exception encountered when trying to process row: ",
                            row,
                            e,
                        )
                        if (
        ...
        self.emails = (
            new_emails  # TODO: implement local storage of issued / sent certificates
        )
    ...
    def make_certs(self, n_attendees=3):
        ...
        for attendee in self.attendee_list[:n_attendees]:
            ...
            docx_file = create_certificate(
                attendee_name,
                self.training_date,
                self.documented_on_date,
